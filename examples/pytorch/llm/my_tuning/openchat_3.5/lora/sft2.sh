...
if [ "$#" -ne 6 ]; then
    echo "错误：需要提供6个参数"
    echo "用法: bash $0 <test_size> <train_ratio> <learning_rate> <lora_rank> <with_or_without_info> <data_version>"
    exit 1
fi
...
torchrun \
    ...
    --output_dir output/openchat_3.5/$with_or_without_info/data$data_version/lr=$learning_rate/lora_rank=$lora_rank/"$output_name" \
    --ddp_backend nccl \
