...
torchrun \
    ...
    --output_dir output/Llama-3-8B-Instruct/$with_or_without_info/data$data_version-split=$split_type-ratio=$train_ratio/dora-r=$lora_rank/"$output_name" \
    --ddp_backend nccl \
    --dataset $custom_train_dataset_path#-1 \
    --val_dataset $custom_val_dataset_path#1 \
