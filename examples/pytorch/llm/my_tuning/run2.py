...
# openchat 3.5
# for lora_rank in [8]:
#     for lr in ["1e-4", "9e-5", "8e-5", "7e-5", "6e-5"]:
...
#                 subprocess.run(
#                     ["bash", f"my_tuning/openchat_3.5/lora/sft2.sh", 
#                     "0.2", f"{train_ratio}", lr, f"{lora_rank}", j, data_version])
...
#     for i in [1.0]:
#         subprocess.run(["bash", f"my_tuning/neural-chat-7b-v3-3/lora/sft.sh", "0.2", f"{i}", lr, data_version])
...
#     for i in [1.0]:
#         subprocess.run(["bash", f"my_tuning/neural-chat-7b-v3-3-Slerp/lora/sft.sh", "0.2", f"{i}", lr, data_version])
...
#     data_version = "3.3"
#     for i in [1.0]:
#         subprocess.run(["bash", f"my_tuning/Mistral-7B-Instruct-v0.2/lora/sft.sh", "0.2", f"{i}", lr, data_version])
